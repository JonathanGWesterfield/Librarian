...
class SQLiteIngestionStoreTests(unittest.TestCase):
    ...
    def test_summary_jobs_can_only_be_claimed_once(self) -> None:
        """Verify workers atomically claim pending summary work.
        If two workers see the same pending row, only the first claim should
        transition it to running so the book is not summarized twice.
        """
        book = BookRecord(
            id="book-1",
            source_path="/books/forward.epub",
        ...
        with SQLiteIngestionStore(self.database_path) as store:
            ...
            first_claim = store.claim_summary_job("summary-job-1", attempts=1)
            second_claim = store.claim_summary_job("summary-job-1", attempts=1)
            running_jobs = store.list_summary_jobs(status="running")
    ...
    def test_summary_job_progress_can_be_persisted_for_running_workers(self) -> None:
        ...
        The UI polls ingestion status while a book is being summarized, so the
        storage row needs to preserve the latest stage, counter, and message.
        """
        book = BookRecord(
            id="book-1",
            source_path="/books/forward.epub",
        ...
        with SQLiteIngestionStore(self.database_path) as store:
            ...
            store.update_summary_job_progress(
                ...
                current_step=2,
                total_steps=5,
                message="Generating summary for Chunks 8-15.",
            )
            running_jobs = store.list_summary_jobs(status="running")

        self.assertEqual(running_jobs[0].current_stage, "chapter")
        self.assertEqual(running_jobs[0].current_step, 2)
        self.assertEqual(running_jobs[0].total_steps, 5)
        self.assertEqual(
