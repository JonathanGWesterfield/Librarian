...
export function App() {
  ...
  const changeScope = (bookId: string | null) => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setChatError(null);
    try {
      const response = await askChat(submittedQuestion, selectedBookId ?? undefined);
  ...
  return <main className="app-shell">
    ...
    <section id="explore" className="hero">
      ...
      <form className="question-form" onSubmit={submit} aria-busy={chatLoading}>
        ...
        <label className="scope-control"><span>Scope</span><select aria-label="Search scope" value={selectedBookId ?? ""} onChange={(event) => changeScope(event.target.value || null)} disabled={booksLoading || chatLoading}><option value="">Whole library</option>{books.map((book) => <option key={book.id} value={book.id}>{bookTitle(book)}</option>)}</select></label>
    ...
    <section id="workspace" className="workspace" aria-labelledby="answer-heading">
      <aside className="filters">
        <div className="filter-heading"><span>Search scope</span>{selectedBook && <button type="button" onClick={() => changeScope(null)}>Whole library</button>}</div>
        <fieldset><legend>Current scope</legend>
          <label><input type="radio" name="scope" checked={!selectedBookId} onChange={() => changeScope(null)} /> Whole library <span>{books.length}</span></label>
        ...
        <div className="demo-note"><span className="note-icon">i</span><p>{selectedBook ? `Answers will use passages from ${bookTitle(selectedBook)} only.` : "Answers search your entire local library."}</p></div>
      </aside>
      <div className="answer-area">
        ...
        {chat ? <>
          <article className="answer-card" aria-live="polite"><div className="answer-number">01</div><div><p className="asked-question">{chat.question}</p><p className="answer-text">{chat.answer}</p><div className="tag-row"><span>{chat.candidate_count} retrieved chunks</span>{selectedBook && <span>{bookTitle(selectedBook)}</span>}</div></div></article>
